...
RSpec.describe DedupeRequests::Controller do
  ...
  describe "the de-dupe set DSL (on/skip)" do
    it "on: sets the exact action list" do
      expect(controller_class(on: %i[create update]).dedupe_requests_actions)
        .to contain_exactly(:create, :update)
    end

    it "on: in a subclass adds to the inherited set" do
      base = controller_class(on: %i[create update])
      sub = Class.new(base) { dedupe_requests on: %i[approve] }
    ...
    it "skip_dedupe_requests removes inherited actions" do
      base = controller_class(on: %i[create update])
      sub = Class.new(base) { skip_dedupe_requests on: %i[update] }
    ...
    it "treats skip: of a never-guarded action as a harmless no-op (across inheritance)" do
      app = Class.new(FakeController) { dedupe_requests on: [:create] }
  ...
  describe "around behavior" do
    it "runs the action and does nothing for an action outside the set" do
      controller = controller_class(on: %i[create]).new(action: "index", request: req)
  ...
  describe "edge cases" do
    ...
    it "passes a per-controller ttl override through to the store" do
      ...
      run(controller_class(on: %i[create], ttl: 30).new(action: "create", request: req))
  ...
  describe "per-action TTL" do
    ...
    let(:app_controller) { Class.new(FakeController) { dedupe_requests on: %i[create update], ttl: 90 } }

    it "applies one line's TTL to every action named on that line" do
      klass = Class.new(FakeController) { dedupe_requests on: %i[create update], ttl: 120 }
    ...
    it "gives each action its own TTL when declared on separate lines" do
      klass = Class.new(FakeController) do
        dedupe_requests on: [:create], ttl: 120
        dedupe_requests on: [:update], ttl: 180
      end
      expect(captured_ttl_for(klass, :create)).to eq(120)
    ...
    it "lets a subclass change the TTL by re-declaring the actions" do
      sub = Class.new(app_controller) { dedupe_requests on: %i[create update], ttl: 30 }
    ...
    it "lets a subclass override one action's TTL, leaving the others inherited" do
      sub = Class.new(app_controller) { dedupe_requests on: [:update], ttl: 180 }
    ...
    it "falls back to the global config TTL when a line has no ttl" do
      ...
      plain = Class.new(FakeController) { dedupe_requests on: [:create] }
